...
def test_score_only_measure_has_no_predict():
    assert not hasattr(ValueOccurrenceCompleteness("color"), "predict")


def test_identical_instance_scores_one(backend):
    frame = make_frame({"color": ["red", "red", "blue", "green"]}, backend)
    measure = ValueOccurrenceCompleteness("color").fit(frame)
    assert measure.score(frame) == pytest.approx(1.0)
...
def test_missing_domain_value_lowers_score(backend):
    ...
    measure = ValueOccurrenceCompleteness("color").fit(train)
...
def test_overrepresentation_is_capped(backend):
    ...
    measure = ValueOccurrenceCompleteness("color").fit(train)
    assert measure.score(test) == pytest.approx(0.75)
...
def test_specified_expected_skips_fit(backend):
    ...
    measure = ValueOccurrenceCompleteness("color", expected={"red": 0.5, "blue": 0.5})
    test = make_frame({"color": ["red", "red", "red", "red"]}, backend)
...
def test_missing_column_raises(backend):
    train = make_frame({"color": ["red"]}, backend)
    with pytest.raises(ValueError, match="not found"):
        ValueOccurrenceCompleteness("size").fit(train)


def test_not_fitted_raises(backend):
    test = make_frame({"color": ["red"]}, backend)
    with pytest.raises(NotResolvedError):
        ValueOccurrenceCompleteness("color").score(test)
